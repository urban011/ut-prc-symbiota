...
class OccurrenceManager{
	...
	public function getSqlWhere(){
        ...
        if(array_key_exists('previous_determinations',$this->searchTermsArr)){
            $sqlWhere .= "AND (d.sciname Like '%".$this->searchTermsArr['previous_determinations']."%') ";
        }
		...
		if(array_key_exists('catnum',$this->searchTermsArr)){
			...
			foreach($catArr as $v){
				if($p = strpos($v,' - ')){
					...
					$vStr = trim($v);
					$inFrag[] = $vStr;
                    // TACC: for PRC we do not want to treat CNs as numeric
...
//					}
			...
			if($inFrag){
				if($isOccid){
					...
					$catWhere .= 'OR (o.catalogNumber REGEXP "'.implode('|',$inFrag).'") ';
				}
			}
...
<!--					title="Click button to advance to the next step" />-->
											...
											<td style="padding:6px">
												<div class="collectiontitle">
													<a href = 'misc/collprofiles.php?collid=<?php echo $collid; ?>'>
														<?php
                                                        $codeStr = ' (';
                                                        if($collName2['instcode']) $codeStr .= $collName2['instcode'];
                                                        if($collName2['instcode'] && $collName2['collcode']) $codeStr .= '-';
	...
	private function readRequestVariables(){
        ...
        if(array_key_exists("previous_determinations",$_REQUEST)){
            $previous_determinations = $this->conn->real_escape_string($_REQUEST["previous_determinations"]);
            if($previous_determinations){
                $str = $previous_determinations;
                $searchArr[] = "previous_determinations:".$str;
                $this->searchTermsArr["previous_determinations"] = $str;
            }
            else{
                unset($this->searchTermsArr["previous_determinations"]);
            }
